...
public class TreeObject {
	...
	*/
		 ...
		 * method to find children of a parent
...
private Node root;
	...
	public void insert(Person s) {
		...
		if(root==null) {
			...
			while(y!=null) {
				x=y;
				if(s.getName().compareToIgnoreCase(y.data.getName())<0) {
					y=y.lc;
				} else {
					y=y.rc;
				}
			}
			if(s.getName().compareToIgnoreCase(x.data.getName())<0) {
				x.lc=n;
			} else {
				x.rc=n;
			}
	...
	public Person getNodeData(Node n) {
		return n.data; 
	}
}
